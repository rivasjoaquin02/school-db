...
var RevealMapbox =
	...
	(function () {
		...
		function addTrack(mapbox, trekUrl) {
			trekId = trekUrl;
			if (mapbox.getLayer(trekId)) return;
			try {
				mapbox.addLayer({
					id: trekId,
					type: "line",
		...
		function goCurrentMapPosition() {
			var newFragment = Reveal.getCurrentSlide().querySelectorAll(
				".fragment.current-fragment"
			);
			if (newFragment.length) {
				newFragment = newFragment[0];
				position = JSON.parse(
					newFragment.getAttribute("data-mapbox-transform-to")
				);
			} else {
				if (
					Reveal.availableFragments().prev === true &&
		...
		Reveal.addEventListener("slidechanged", function (event) {
			if (!event.currentSlide.hasAttribute("data-mapbox")) return;
			Reveal.getCurrentSlide().mapbox.resize();
			if (event.currentSlide.hasAttribute("data-mapbox-trek"))
				addTrack(
					Reveal.getCurrentSlide().mapbox,
					Reveal.getCurrentSlide().getAttribute("data-mapbox-trek")
				);
