...
export const accessMethodType = pgEnum("access_method_type", [
	"member card",
	"provisional pass",
]);
...
export const penaltyType = pgEnum("penalty_type", [
	"late fees",
...
export const mapType = pgEnum("map_type", [
	"topographic",
	"road",
	"thematic",
	"geologic",
...
export const techniqueType = pgEnum("technique_type", [
	"oil",
	"acrylic",
	"watercolor",
	"pastel",
...
export const emailCollection = pgTable("email_collection", {
	...
	idCollection: varchar("id_collection", { length: 10 }).references(
...
export const library = pgTable(
	...
	(table) => {
		return {
			libraryNameLibraryKey: unique("library_name_library_key").on(
				table.nameLibrary
			),
		};
	}
...
export const collection = pgTable(
	"collection",
	{
		idCollection: varchar("id_collection", { length: 10 })
			.primaryKey()
			.notNull(),
		idRoom: varchar("id_room", { length: 20 }).references(
			() => room.idRoom
		),
		nameCollection: varchar("name_collection", { length: 255 }).notNull(),
		descriptionCollection: text("description_collection"),
		typeCollection: collectionType("type_collection")
			.default("general")
			.notNull(),
	},
	(table) => {
		return {
			collectionNameCollectionKey: unique(
				"collection_name_collection_key"
			).on(table.nameCollection),
		};
	}
);
...
export const phoneRoom = pgTable("phone_room", {
	phoneNumber: varchar("phone_number", { length: 20 })
		...
		.notNull()
		.references(() => phone.phoneNumber),
	idRoom: varchar("id_room", { length: 20 })
		.notNull()
...
export const manuscript = pgTable("manuscript", {
	idDocument: integer("id_document")
		.primaryKey()
		.notNull()
...
export const service = pgTable("service", {
	idService: bigserial("id_service", { mode: "bigint" })
		.primaryKey()
		.notNull(),
...
export const collectionDocument = pgTable(
	"collection_document",
	{
		idCollection: varchar("id_collection", { length: 10 })
			.notNull()
	...
	(table) => {
		return {
			collectionDocumentPkey: primaryKey({
				columns: [table.idCollection, table.idDocument],
				name: "collection_document_pkey",
			}),
		};
	}
);

export const serviceRoom = pgTable(
	"service_room",
	{
		idService: integer("id_service").notNull(),
		idRoom: varchar("id_room", { length: 20 })
	...
	(table) => {
		return {
			...
			serviceRoomPkey: primaryKey({
				columns: [table.idService, table.idRoom],
				name: "service_room_pkey",
			}),
		};
	}
);

export const authorDocument = pgTable(
	"author_document",
	{
		idAuthor: integer("id_author")
			.notNull()
	...
	(table) => {
		return {
			authorDocumentPkey: primaryKey({
				columns: [table.idAuthor, table.idDocument],
				name: "author_document_pkey",
			}),
		};
	}
);

export const serviceMember = pgTable(
	"service_member",
	{
		idService: integer("id_service").notNull(),
		idMember: integer("id_member")
	...
	(table) => {
		return {
			...
			serviceMemberPkey: primaryKey({
				columns: [table.idService, table.idMember],
				name: "service_member_pkey",
			}),
		};
	}
);

export const loanResearcher = pgTable(
	"loan_researcher",
	{
		idService: integer("id_service").notNull(),
		idDocument: integer("id_document")
	...
	(table) => {
		return {
			...
			loanResearcherPkey: primaryKey({
				columns: [table.idService, table.idDocument, table.idMember],
				name: "loan_researcher_pkey",
			}),
		};
	}
);

export const loanProfessional = pgTable(
	"loan_professional",
	{
		idService: integer("id_service").notNull(),
		idDocument: integer("id_document")
	...
	(table) => {
		return {
			...
			loanProfessionalPkey: primaryKey({
				columns: [table.idService, table.idDocument, table.idMember],
				name: "loan_professional_pkey",
			}),
		};
	}
);

export const loanLibrary = pgTable(
	"loan_library",
	{
		idService: integer("id_service").notNull(),
		idDocument: integer("id_document")
		...
		idLibrary: integer("id_library")
			.notNull()
			.references(() => library.idLibrary),
		idLibrary2: integer("id_library2")
			.notNull()
			.references(() => library.idLibrary),
	},
	(table) => {
